...
def best_pair(pool: list[dict], lo: float, hi: float, exclude_games: set[str],
    ...
    for i in range(len(cand)):
        for j in range(i + 1, len(cand)):
            ...
            # score: joint model prob, with a nudge for cross-sport diversification
            score = (a["modelProbability"] * b["modelProbability"]) + (0.05 if cross and prefer_cross_sport else 0)
            if score > best_score:
...
def build(date: str) -> dict:
    ...
    # Lane A — lower variance: highest joint model prob near the lower end of the window.
    a = best_pair(pool, 1.80, 2.15, set(), set(), prefer_cross_sport=True)
    if not a:
        a = best_pair(pool, 1.70, 2.30, set(), set(), prefer_cross_sport=True)
    if not a:
    ...
    # Lane B — differentiated, slightly higher return; shares no game with Lane A.
    b = best_pair(pool, 2.05, 2.45, used_games, used_keys, prefer_cross_sport=True)
    if not b:
        b = best_pair(pool, 1.85, 2.45, used_games, used_keys, prefer_cross_sport=True)
    if not b:
