...
const outDir = path.join(ROOT, "data/internal/research/nfl/joint-v2-forward");
...
for (const fc of events) {
  ...
  for (const abbr of [fc.home.abbr, fc.away.abbr]) {
    ...
    const chart = sourceDepth.state === "PROJECTED_DEPTH_STARTER" ? { ...sourceDepth, sourceEspnId: sourceDepth.playerId, playerId: `nfl-athlete-${sourceDepth.playerId}` } : sourceDepth;
    const conditioned = conditionQuarterbackShares(players, chart, 0.9577);
    const reconciled = reconcileJointRoster(conditioned.players);
    playersByTeam[abbr] = reconciled.players;
    inputNotes[abbr] = { chart, conditioningApplied: conditioned.applied, conditioningReason: conditioned.reason, adjustments: reconciled.adjustments };
  }
  ...
  const output = path.join(outDir, `${fc.providerEventId}-${inputHash.slice(0, 20)}.json`);
  const doc = { artifact: "nfl-joint-v2-forward-shadow", dataClass: "PRIVATE_RESEARCH", generatedAt: NOW, kickoffUtc: fc.kickoffUtc,
    sourceHashes: hashes, inputHash, inputs, inputNotes, roleEvidenceGeneratedAt: roles.generatedAt,
    limitations: ["Not public eligible; no markets or recommendations", "Historical role snapshot is not current actives", "Depth chart is not guaranteed participation", "Partial offensive model; other scoring remains unspecified"], simulation: sim };
