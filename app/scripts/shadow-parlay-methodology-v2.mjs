...
// ---------------------------------------------------------------------------
// Config — deterministic, AUTO-DISCOVERED slate lists (so new settled slates
// are tracked with no code edit). May 25/26 are BANNED; the public era starts
// 2026-05-27. A date is "settled" only when it has a graded optimizer file, so
// pending slates (e.g. an unsettled June-2) are never counted.
// ---------------------------------------------------------------------------
const PUBLIC_ERA_START = "2026-05-27"; // public era begins here.
const EXCLUDED_DATES = new Set(["2026-05-25", "2026-05-26"]); // banned public rates.
...
/** Settled public-era dates: a graded optimizer file exists, the date is in the
 *  public era (>= 2026-05-27), and it is not a banned May 25/26 date. */
function discoverSettledDates() {
  return datesInDir(resolve(DATA, "parlays", "optimizer-graded")).filter(
    (d) => d >= PUBLIC_ERA_START && !EXCLUDED_DATES.has(d),
  );
}
/** Graded dates that are deliberately EXCLUDED (banned 25/26 or pre-era). */
function discoverExcludedSettledDates() {
  return datesInDir(resolve(DATA, "parlays", "optimizer-graded")).filter(
    (d) => d < PUBLIC_ERA_START || EXCLUDED_DATES.has(d),
  );
}
/** Active slate = latest date with an optimizer board (availability view). */
function discoverActiveDate() {
  const dates = datesInDir(resolve(DATA, "parlays", "optimizer"));
  return dates.length ? dates[dates.length - 1] : (discoverSettledDates().slice(-1)[0] ?? null);
}

const SETTLED_PUBLIC_DATES = discoverSettledDates();
const EXCLUDED_SETTLED_DATES = discoverExcludedSettledDates();
const ACTIVE_DATE = discoverActiveDate();

// Minimum decided-leg sample before a gated bucket's signal is usable.
const SAMPLE_THRESHOLD = 40;

// CLI: `--write-report` persists a deterministic markdown report (no wall-clock
// stamp, so re-runs on the same data are byte-identical). Default = read-only.
const WRITE_REPORT = (typeof process !== "undefined" ? process.argv : []).includes("--write-report");
const REPORT_PATH = resolve(__dirname, "..", "..", "docs", "audits", "methodology-v2-shadow-latest.md");
...
// ---------------------------------------------------------------------------
// Deterministic markdown report (only written with --write-report). No
// wall-clock stamp, so re-runs on identical data produce an identical file.
// Never writes betting/product data — only this docs snapshot.
// ---------------------------------------------------------------------------
function buildReportMarkdown(ctx) {
  const { agg, pubAgg, availTrue, buildLive, buildRelaxed, bank, juneGraded } = ctx;
  const N = (x) => x.w + x.l;
  const rate = (x) => `${x.w}/${N(x)}=${pct(x.w, N(x))}`;
  const fiveN = N(agg.l5_5of5);
  const lowN = N(agg.lowElig);
  const met = (n) => (n >= SAMPLE_THRESHOLD ? "yes" : "NO");
  const thresholdsMet = fiveN >= SAMPLE_THRESHOLD && lowN >= SAMPLE_THRESHOLD;

  const lines = [];
  lines.push("# Methodology v2 — Shadow Tracking Report (auto-generated)");
  lines.push("");
  lines.push("> Generated by `app/scripts/shadow-parlay-methodology-v2.mjs --write-report`.");
  lines.push("> READ-ONLY evidence snapshot — **no live behavior, no betting/product data**.");
  lines.push("> Deterministic (no wall-clock); re-run after each nightly settle to track");
  lines.push("> sample growth. **v2 remains SHADOW-ONLY.** True L5/L10 from the board full");
  lines.push("> series (MLB). Pushes & pending excluded; `edgePct`/`confidence` never used.");
  lines.push("");
  lines.push("## Coverage");
  lines.push(`- **Settled public-era slates (${SETTLED_PUBLIC_DATES.length}):** ${SETTLED_PUBLIC_DATES.join(", ") || "(none)"}`);
  lines.push(`- **Excluded** (banned May 25/26 or pre-era): ${EXCLUDED_SETTLED_DATES.join(", ") || "(none)"}`);
  lines.push(`- **Active slate** (latest optimizer): ${ACTIVE_DATE} — settled? ${juneGraded ? "yes" : "no (availability only)"}`);
  lines.push(`- Public era starts \`${PUBLIC_ERA_START}\`. Sample threshold: **${SAMPLE_THRESHOLD}** decided legs per gated bucket.`);
  lines.push("");
  lines.push("## Sample sizes (decided MLB legs) & threshold");
  lines.push("| Bucket | Decided N | ≥40 met? |");
  lines.push("|--------|------:|:--:|");
  lines.push(`| all (model-selected) | ${N(agg.all)} | n/a |`);
  lines.push(`| **L5 5/5** | ${fiveN} | ${met(fiveN)} |`);
  lines.push(`| L5 4/5+ | ${N(agg.l5_4plus)} | n/a |`);
  lines.push(`| **Low-eligible** | ${lowN} | ${met(lowN)} |`);
  lines.push(`| Bank-eligible (L10 8/10+) | ${N(agg.bankElig)} | n/a |`);
  lines.push("");
  lines.push("## Historical hit rates (true L5, MLB)");
  lines.push("| Leg group | Hit rate | N |");
  lines.push("|-----------|------:|------:|");
  lines.push(`| all (model-selected) | ${pct(agg.all.w, N(agg.all))} | ${N(agg.all)} |`);
  lines.push(`| L5 5/5 | ${pct(agg.l5_5of5.w, fiveN)} | ${fiveN} |`);
  lines.push(`| L5 4/5 | ${pct(agg.l5_4of5.w, N(agg.l5_4of5))} | ${N(agg.l5_4of5)} |`);
  lines.push(`| L5 4/5+ | ${pct(agg.l5_4plus.w, N(agg.l5_4plus))} | ${N(agg.l5_4plus)} |`);
  lines.push(`| L5 3/5 or worse | ${pct(agg.l5_3orWorse.w, N(agg.l5_3orWorse))} | ${N(agg.l5_3orWorse)} |`);
  lines.push(`| Low-eligible (5/5 & ≤−150) | ${pct(agg.lowElig.w, lowN)} | ${lowN} |`);
  lines.push(`| Bank-eligible (L10 ≥8/10) | ${pct(agg.bankElig.w, N(agg.bankElig))} | ${N(agg.bankElig)} |`);
  lines.push("");
  lines.push(`Published cards (graded publicRiskSections): slips ${pubAgg.slips}, leg ${pct(pubAgg.legW, pubAgg.legW + pubAgg.legL)}, slip win of decided ${pct(pubAgg.slipW, pubAgg.slipW + pubAgg.slipL)} (${pubAgg.slipW}W/${pubAgg.slipL}L).`);
  lines.push("");
  lines.push("## Per-slate (true L5)");
  lines.push("| date | MLBdec | all | L5 5/5 | L5 4/5+ | LowElig |");
  lines.push("|------|------:|------:|------:|------:|------:|");
  for (const d of SETTLED_PUBLIC_DATES) {
    const p = legPredictiveness(d);
    if (!p) {
      lines.push(`| ${d} | (no graded file) | | | | |`);
      continue;
    }
    const b = p.buckets;
    lines.push(`| ${d} | ${p.mlbDecided} | ${rate(b.all)} | ${rate(b.l5_5of5)} | ${rate(b.l5_4plus)} | ${rate(b.lowElig)} |`);
  }
  lines.push("");
  lines.push(`## Active-slate feasibility (${ACTIVE_DATE}, true form)`);
  lines.push(`- Eligible legs: Low **${availTrue.lowEligible}**, Med/High/Longshot **${availTrue.medHiLsEligible}**, Bank (L10 ≥8/10) **${availTrue.bankEligible}**.`);
  lines.push(`- Cards under **live #241 caps**: low ${buildLive.counts.low}/${TARGETS.low}, med ${buildLive.counts.medium}/${TARGETS.medium}, high ${buildLive.counts.high}/${TARGETS.high}, longshot ${buildLive.counts.longshot}/${TARGETS.longshot} (total ${buildLive.counts.total}).`);
  lines.push(`- Cards under **relaxed caps** (headroom only): low ${buildRelaxed.counts.low}, med ${buildRelaxed.counts.medium}, high ${buildRelaxed.counts.high}, longshot ${buildRelaxed.counts.longshot} (total ${buildRelaxed.counts.total}).`);
  lines.push(`- Bank Builder qualifying card exists? ${bank.plus100CardsFound > 0 ? "yes" : "no"}.`);
  lines.push("");
  lines.push("## Recommendation");
  if (thresholdsMet) {
    lines.push(`- **Sample threshold MET** (L5 5/5 N=${fiveN}, Low N=${lowN} ≥ ${SAMPLE_THRESHOLD}). The operator may revisit the decision matrix in \`METHODOLOGY_V2_IMPLEMENTATION_DECISION_2026-06-02.md\` — still **no win-rate claim**, no padding, and the #241 cap-vs-target decision remains required before any live wiring.`);
  } else {
    lines.push(`- **KEEP v2 SHADOW-ONLY — gather more settled slates.** L5 5/5 N=${fiveN} and/or Low N=${lowN} are below the ${SAMPLE_THRESHOLD}-decided-leg threshold; the signal is not yet conclusive. Re-run after each nightly settle.`);
  }
  lines.push(`- Data plumbing: ${ctx.flips === 0 ? "RESOLVED (0 Low-eligibility flips on the active slate)" : `STALE on the active slate (${ctx.flips} flips) — regenerate it first`}.`);
  lines.push("- L5 4/5+ historically shows no lift over the model-selected baseline; do not gate Medium/High/Longshot on it without stronger evidence.");
  lines.push("");
  lines.push("*This file is overwritten by the script. Do not hand-edit. See the runbook: `METHODOLOGY_V2_SHADOW_TRACKING_RUNBOOK.md`.*");
  lines.push("");
  return lines.join("\n");
}
...
function main() {
  ...
  console.log(`Settled public-era slates (auto-discovered, ${SETTLED_PUBLIC_DATES.length}): ${SETTLED_PUBLIC_DATES.join(", ") || "(none)"}`);
  console.log(`Excluded settled dates (banned/pre-era): ${EXCLUDED_SETTLED_DATES.join(", ") || "(none)"}`);
  console.log(`Active slate (latest optimizer): ${ACTIVE_DATE}`);

  // --- Active slate availability + feasibility ---
  hr(`PHASE 0 — ${ACTIVE_DATE} settlement state & data sources`);
  const juneGraded = existsSync(gradedPath(ACTIVE_DATE));
  console.log(`${ACTIVE_DATE} graded file present? ${juneGraded ? "YES (settled — outcomes usable)" : "NO (UNSETTLED — availability only; no outcomes)"}`);
  const opt = loadJSON(optimizerPath(ACTIVE_DATE));
  const boardIdx = buildBoardIndex(ACTIVE_DATE);
  console.log(`${ACTIVE_DATE} optimizer legPool present? ${opt?.legPool?.legs ? `YES (${opt.legPool.legs.length} legs)` : "NO"}`);
  console.log(`${ACTIVE_DATE} board index entries (player|market -> full series): ${boardIdx.size}`);

  hr(`PHASE A — ${ACTIVE_DATE} candidate leg availability (TRUE form vs TRUNCATED)`);
  const rawLegs = opt?.legPool?.legs ?? [];
  const enriched = rawLegs.map((l) => enrichLeg(l, boardIdx));
  const availTrue = legAvailability(enriched);
  printAvailabilityTable(`${ACTIVE_DATE} TRUE form (board full series)`, availTrue);
  ...
  hr(`PHASE B — ${ACTIVE_DATE} v2 construction feasibility (heuristic greedy)`);
  ...
  hr(`PHASE D — ${ACTIVE_DATE} Bank Builder feasibility (true L10 8/10+)`);
  ...
  const bankN = agg.bankElig.w + agg.bankElig.l;
  console.log(`Sample sizes (decided MLB legs): all=${agg.all.w + agg.all.l}, L5 5/5=${fiveN}, L5 4/5+=${fourPlusN}, Low-elig=${lowN}, Bank-elig=${bankN}`);
  const SMALL = SAMPLE_THRESHOLD;
  const warn = [];
  ...
  console.log("");
  console.log(`Feasibility (${ACTIVE_DATE}, TRUE form):`);
  ...
  if (WRITE_REPORT) {
    const md = buildReportMarkdown({ agg, pubAgg, availTrue, buildLive, buildRelaxed, bank, juneGraded, flips });
    mkdirSync(dirname(REPORT_PATH), { recursive: true });
    writeFileSync(REPORT_PATH, md, "utf8");
    console.log(`\n[--write-report] wrote ${REPORT_PATH}`);
  } else {
    console.log("\n(tip: pass --write-report to persist a deterministic markdown snapshot)");
  }
}
