...
export const metadata = {
  title: "NHL Results · GameTime Picks",
  description:
    "NHL model audit · pending. No NHL projections have been graded yet because the model board is still warming up.",
};

export default function NhlResultsPlaceholderPage() {
  ...
  return (
    <div className="vault-page-shell px-4 sm:px-8 py-8 sm:py-14 overflow-x-hidden">
      ...
      <section className="reveal">
        ...
        >
          NHL model audit · pending first settlement
        </div>
        <h1
          className="mt-2 vault-display-h2"
          style={{ color: "var(--vault-text)" }}
        >
          Grades land here after the first NHL slate finals.
        </h1>
        <p
          className="mt-3 max-w-2xl text-[13px] leading-relaxed"
          style={{ color: "var(--vault-text-mute)" }}
        >
          The NHL model board has to ship first — projections logged at
          generation time and then graded against the verified box
          score after each game completes. Today the only NHL surface
          live is the schedule. We will not surface any NHL hit rate
          before the audit is real.
        </p>
      </section>
      ...
      <section className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          ...
          NHL Odds API integration for shots on goal + goalie saves, a
          per-skater + per-goalie last-N game log loader from the free
          NHL API, and a settlement job that grades each logged lean
          against the boxscore. Once live, NHL feeds the cross-sport
          overall hit rate just like NBA and MLB do today.
        </div>
        <div
          className="rounded-[6px] px-4 py-4 text-[12px] leading-relaxed"
          style={{
        ...
        >
          ...
          shows the live NBA + MLB cross-sport audit. NHL is
