import Link from "next/link";
import {
  getAvailableIplScheduleDates,
  getIplScheduleForDate,
} from "@/lib/data-ipl";
import IplBoardBody from "@/components/ipl/ipl-board-body";
import IplSectionTabs from "@/components/ipl/ipl-section-tabs";

export function generateStaticParams() {
  return getAvailableIplScheduleDates().map((date) => ({ date }));
}

export function generateMetadata({ params }: { params: { date: string } }) {
  return {
    title: `IPL Model Board · ${params.date} · GameTime Picks`,
    description: `IPL slate for ${params.date}. Schedule loaded; per-player stats provider pending.`,
  };
}

const ISO_DATE = /^\d{4}-\d{2}-\d{2}$/;

export default function IplBoardForDatePage({
  params,
}: {
  params: { date: string };
}) {
  const date = params.date;
  if (!ISO_DATE.test(date)) return <DateNotFound date={date} />;
  const schedule = getIplScheduleForDate(date);
  if (schedule.scheduleSource === "unavailable") {
    return <DateNotFound date={date} />;
  }
  return <IplBoardBody date={date} />;
}

function DateNotFound({ date }: { date: string }) {
  const dates = getAvailableIplScheduleDates();
  const nearest =
    dates.find((d) => d >= date) ?? dates[dates.length - 1] ?? null;
  return (
    <div className="vault-page-shell px-4 sm:px-8 py-8 sm:py-14 overflow-x-hidden">
      <div className="mb-6">
        <IplSectionTabs />
      </div>
      <section className="reveal">
        <div
          className="font-mono uppercase tracking-[0.16em]"
          style={{ color: "var(--vault-warn)", fontSize: 11 }}
        >
          IPL board · date unavailable
        </div>
        <h1
          className="mt-2 vault-display-h2"
          style={{ color: "var(--vault-text)" }}
        >
          No IPL data for {date}.
        </h1>
        <p
          className="mt-3 max-w-2xl text-[13px] leading-relaxed"
          style={{ color: "var(--vault-text-mute)" }}
        >
          The IPL schedule for this date is not on disk. The next refresh
          will pull the rolling window when the slate posts.
        </p>
        <div className="mt-6 flex flex-wrap gap-2">
          <Link
            href="/ipl/board"
            className="vault-glow-hover rounded-[3px]"
            style={{
              padding: "10px 14px",
              border: "1px solid rgba(212, 175, 55, 0.30)",
              background: "rgba(212, 175, 55, 0.06)",
              color: "var(--vault-gold-bright)",
              textDecoration: "none",
              fontSize: 12,
              fontFamily: "var(--font-mono)",
              letterSpacing: "0.12em",
              textTransform: "uppercase",
            }}
          >
            Latest available slate →
          </Link>
          {nearest && nearest !== date && (
            <Link
              href={`/ipl/board/${nearest}`}
              className="vault-glow-hover rounded-[3px]"
              style={{
                padding: "10px 14px",
                border: "1px solid var(--vault-border)",
                background: "rgba(7, 11, 26, 0.45)",
                color: "var(--vault-text-mute)",
                textDecoration: "none",
                fontSize: 12,
                fontFamily: "var(--font-mono)",
                letterSpacing: "0.12em",
                textTransform: "uppercase",
              }}
            >
              Nearest date · {nearest} →
            </Link>
          )}
        </div>
      </section>
    </div>
  );
}
