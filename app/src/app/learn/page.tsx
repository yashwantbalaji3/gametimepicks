...
function Concept({ term, children }: { term: string; children: React.ReactNode }) {
  return (
    <div className="rounded-[10px] px-4 py-4 flex flex-col gap-1.5" style={{ background: "rgba(7,11,26,0.55)", border: "1px solid var(--vault-border)" }}>
    ...
    <div className="vault-page-shell px-4 sm:px-8 py-8 sm:py-12 overflow-x-hidden flex flex-col gap-9">
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {TIERS.map((t) => (
            <div key={t.tier} className="flex items-start gap-3 rounded-[8px] px-4 py-3" style={{ background: "rgba(7,11,26,0.55)", border: "1px solid var(--vault-border)" }}>
              <span className="font-mono uppercase tracking-[0.1em] px-2 py-0.5 rounded-full shrink-0 mt-0.5" style={{ color: t.tone, border: `1px solid ${t.tone}`, fontSize: 9 }}>{t.tier}</span>
      ...
      <section id="sports" className="scroll-mt-16 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {SPORTS.map((s) => (
            <div key={s.name} className="rounded-[8px] px-4 py-3" style={{ background: "rgba(7,11,26,0.55)", border: "1px solid var(--vault-border)" }}>
      ...
      <section className="rounded-[10px] px-4 py-4" style={{ background: "rgba(7,11,26,0.45)", border: "1px solid var(--vault-border)" }}>
