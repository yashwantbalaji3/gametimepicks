...
const TONE: Record<string, string> = {
  UPCOMING: "var(--gtp-success-on-dark)",
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 font-mono text-[10px]" style={{ color: "var(--vault-text-faint)" }}>
        ...
        <span style={{ color: index.contradictions.length ? "var(--gtp-bank-heat)" : "var(--gtp-success-on-dark)" }}>
      ...
      {lane ? (
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 font-mono text-[10px]" style={{ color: "var(--vault-text-mute)" }}>
          {Object.entries(lane.freshness).map(([k, f]) => (
            <span key={k}>
              {k} <span style={{ color: f.state === "FRESH" ? "var(--gtp-success-on-dark)" : "var(--gtp-bank-heat)" }}>{f.state}</span>
            </span>
          ))}
            ...
            <tbody>
              {events.map((e) => (
                <tr key={e.canonicalEventId}>
                  ...
                  <td style={{ ...td, fontFamily: "var(--font-mono, monospace)", color: e.hasMarket ? "var(--gtp-success-on-dark)" : "var(--gtp-bank-heat)" }}>
      ...
      {eligibility ? (
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 font-mono text-[10px]" style={{ color: "var(--vault-text-mute)" }}>
          ...
          {eligibility.products.map((p) => (
            <span key={p.product}>
              {p.product} <span style={{ color: p.eligible ? "var(--gtp-success-on-dark)" : "var(--vault-text-faint)" }}>{p.state}</span>
            </span>
          ))}
        </div>
      ) : (
