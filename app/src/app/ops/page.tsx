...
function LaneRow({ l }: { l: Lane }) {
  ...
  return (
    <div className="rounded-lg px-3 py-2" style={{ border: `1px solid ${live ? "color-mix(in srgb, var(--vault-accent) 40%, transparent)" : "var(--vault-rule)"}`, background: live ? "color-mix(in srgb, var(--vault-accent) 6%, transparent)" : "color-mix(in srgb, var(--vault-wash-base) 1.5%, transparent)" }}>
    ...
    <div className="mx-auto flex max-w-[900px] flex-col gap-4 px-4 py-8 sm:px-6 sm:py-10">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card title="Company health">
          ...
          {s.warnings && s.warnings.length ? (
            <div className="mt-3 flex flex-col gap-1">{s.warnings.map((w, i) => <div key={i} className="rounded px-2 py-1 text-[11px]" style={{ background: "color-mix(in srgb, var(--vault-accent) 8%, transparent)", color: "var(--gtp-bank-heat)" }}>⚠ {w}</div>)}</div>
      ...
      <Card title={`Production health · slate ${growth.health.slateFreshness.toUpperCase()}`}>
        {growth.health.incident ? (
          <div className="mb-2 rounded px-2 py-1.5 text-[11.5px]" style={{ background: "color-mix(in srgb, var(--vault-accent) 10%, transparent)", color: "var(--gtp-bank-heat)" }}>⚠ INCIDENT — {growth.health.incident}</div>
        ) : (
      ...
      <Card title={`Social operations · ${opsBoard.launchable}/${opsBoard.slots.length} launchable · review only (never auto-posted)`}>
        <div className="flex flex-col gap-2">
          {opsBoard.slots.map((slot) => (
            <div key={slot.slot} className="rounded-lg px-3 py-2" style={{ border: `1px solid ${slot.blocked ? "color-mix(in srgb, var(--vault-accent) 40%, transparent)" : "var(--vault-rule)"}`, background: slot.blocked ? "color-mix(in srgb, var(--vault-accent) 5%, transparent)" : "color-mix(in srgb, var(--vault-wash-base) 1.5%, transparent)" }}>
              <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="flex flex-col gap-1.5">
          {GATES.map(([k, cmd]) => (
            <div key={k} className="flex flex-col gap-0.5 sm:flex-row sm:items-baseline sm:gap-3">
              ...
              <code className="overflow-x-auto rounded px-2 py-1 font-mono text-[10px]" style={{ background: "color-mix(in srgb, var(--vault-ink-black) 30%, transparent)", color: "var(--vault-text-faint)" }}>{cmd}</code>
            </div>
          ))}
        </div>
