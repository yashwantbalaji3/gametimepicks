...
test("empty pool returns empty slips with 'empty-pool' reason", () => {
  const result = generateCustomParlaysFromPool([]);
...
test("filters that eliminate everything return 'no-legs-after-filters'", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { team: "NYK" });
...
test("NBA mode returns NBA-only legs", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { sport: "nba", risk: "balanced", count: 1 });
...
test("MLB mode returns MLB-only legs", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { sport: "mlb", risk: "balanced", count: 1 });
  assert.equal(out.slips.length, 1);
  for (const l of out.slips[0].legs) {
...
test("Mixed mode requires ≥ 2 sports per slip", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { sport: "multi", risk: "balanced", count: 1 });
  assert.equal(out.slips.length, 1);
...
test("Mixed mode returns no slips when only one sport in pool", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { sport: "multi" });
...
test("team filter restricts to one team", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { team: "OKC", risk: "balanced", count: 1 });
  assert.equal(out.slips.length, 1);
  for (const l of out.slips[0].legs) {
...
test("gameId filter restricts to one game", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { gameId: "g1", risk: "balanced", count: 1 });
  assert.equal(out.slips.length, 1);
  for (const l of out.slips[0].legs) {
...
test("DNP guard: Conservative excludes NBA legs with recent10Count < 7", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { risk: "conservative", count: 5 });
...
test("DNP guard: MLB Conservative needs len(recentSeries) >= 5", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { risk: "conservative", sport: "mlb", count: 5 });
...
test("Conservative slip has exactly 2 legs", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { risk: "conservative", count: 1 });
...
test("no slip ever exceeds the risk's max legs (4 for Longshot)", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { risk: "aggressive", count: 1 });
...
test("no slip contains two legs from the same player", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { risk: "balanced", count: 1 });
  const seen = new Set();
  for (const l of out.slips[0].legs) {
...
test("two generated slips have different anchor players when alternatives exist", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { risk: "balanced", count: 2 });
...
test("returns 'below-target' when pool can build < count slips", () => {
  ...
  const out = generateCustomParlaysFromPool(pool, { risk: "balanced", count: 5 });
