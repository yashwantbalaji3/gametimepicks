/** Private TD mean-preservation candidate. V1/V2 source and receipts remain immutable.
 ...
 * Predictive acceptance is separate from these accounting identities. See CODEX_NFL_JOINT_V3_CONTRACT.
 */
import { drawMeanPreservingBoundedCount } from "./bounded-count-mean.mjs";
...
export const NFL_JOINT_SIM_ID = "nfl-joint-sim-v3";
export const NFL_JOINT_SIM_VERSION = 3;
...
export function simulateJointGame(args) {
  ...
  const base = { engineId: NFL_JOINT_SIM_ID, version: 3, dataClass: "PRIVATE_RESEARCH", providerEventId: event?.providerEventId ?? null, teamAbbr };
  ...
  const rng = mulberry32(fnv1a(`nfl-joint-sim-v2::${event.providerEventId}::${artifactDate}::${teamAbbr}`));
  ...
  for (let i = 0; i < runs; i++) {
    ...
    const drawnOffTd = drawMeanPreservingBoundedCount(rng, Math.max(0, bridge.lambdaIntercept + bridge.lambdaPerPoint * own), Math.floor(own / 6));
    let offTd = 0;
    ...
    for (const p of allPlayers) {
      ...
      if (b.completions > b.passAtt || b.receptions > b.targets || b.recTd > b.receptions || b.rushTd > b.carries || !fields.every(k => Number.isInteger(b[k]) && b[k] >= 0)) throw new Error(`joint-v3 invalid player draw ${i}:${p.playerId}`);
      for (const k of fields) acc[p.playerId][k].push(b[k]);
    }
    if (sum("passAtt") !== passAtt || sum("targets") !== passAtt || sum("carries") !== rushAtt || sum("passYds") !== sum("recYds") || sum("completions") !== sum("receptions") || sum("passTd") !== sum("recTd") || sum("tdCount") !== offTd) throw new Error(`joint-v3 accounting failure draw ${i}`);
