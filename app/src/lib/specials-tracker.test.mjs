...
test("World Cup Specials are SEPARATE from core / moonshot / crown (no blending)", () => {
  ...
  assert.deepEqual((portfolio.protectedFold?.base?.record ?? portfolio.record), { wins: 19, losses: 14, voids: 0, pending: 0 }, "core record reflects only the BB settlement, not specials (19-14: Lane A won its July-6 cycle-8 Step-1 and July-7 Step-2)");
  assert.equal(portfolio.openExposure, 0, "core exposure unchanged by specials ($0 — both lanes settled)");
  assert.deepEqual((portfolio.moonshot?.legacy ?? portfolio.moonshot).record, { wins: 0, losses: 1, voids: 0, pending: 0 }, "moonshot record unchanged");
