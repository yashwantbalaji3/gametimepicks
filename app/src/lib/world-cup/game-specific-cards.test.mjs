...
test("engine game-specific cards map to the correct WC fixture (Switzerland-Bosnia), bucketed by risk", () => {
  ...
  const g = getGameSpecificCardsForGame({ matchId: d.matchId, homeTeam: d.homeTeam, awayTeam: d.awayTeam });
...
test("cards never leak across fixtures (Switzerland cards != Canada cards)", () => {
  ...
  const swCards = new Set(getGameSpecificCardsForGame({ matchId: sw.matchId, homeTeam: sw.homeTeam, awayTeam: sw.awayTeam }).cards.map((c) => c.parlayId));
  const caCards = getGameSpecificCardsForGame({ matchId: ca.matchId, homeTeam: ca.homeTeam, awayTeam: ca.awayTeam }).cards.map((c) => c.parlayId);
