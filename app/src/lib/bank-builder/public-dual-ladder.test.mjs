...
test("Lane A public ladder after the July-6 cycle-8 restart: ACTIVE fresh Step-1 path, no cleared/prior legs surfaced", () => {
  ...
  // on July-6 the operator approved a fresh cycle-8 restart, so the lane is ACTIVE on a fresh $100 Step-1. None
  // of the prior cycles' (won OR lost) steps are read into the public view; only Mr. Dub carries that history.
  ...
  assert.equal(v.steps.filter((s) => s.status === "cleared").length, 0, "no cleared rung (fresh cycle-8 starting path)");
...
test("DEMO: post July-6 activation both lanes surface a defined path (never a blank actionable row): Lane A ACTIVE fresh Step-1, Lane B clean queued Step-1, no prior-cycle history surfaced", () => {
  // Post July-6 activation: Lane A RESTARTED (cycle 8) → ACTIVE fresh $100 Step-1 path; Lane B is a NO-PLAY
  // (stopped) → a CLEAN queued Step-1 starting path. Both surface defined paths, never a blank actionable row;
  // no cleared rungs and no prior-cycle history (the July-5 losses) bleed in — that history is tracked in Mr. Dub.
...
test("DEMO: Ladder #2 banked + July-5 settlement (both lanes lost) → legacy portfolio carries no exposure (core $0; moonshot settled → 0)", () => {
  ...
  // LOST both lanes July-5. For July-6 only Lane A RESTARTED (ACTIVE cycle 8, fresh $100 Step-1); Lane B is a
  ...
  assert.equal(bbRaw.laneA.laneStatus, "active", "Lane A active — cycle-8 restart (fresh Step 1)");
