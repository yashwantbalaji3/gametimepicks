...
test("Lane A public ladder after settlement: Step 1 + Step 2 cleared (won), advanced toward Step 3, no lost step", () => {
  ...
  assert.equal(v.currentStatus, "active"); // Step 2 WON → riding to Step 3
  ...
  // Step 3 is now PLACED (Japan ML + Egypt ML) → "active" with its card riding $601.56. Steps 4-5 upcoming.
  assert.equal(v.steps[2].status, "active", "Step 3 placed → active card riding");
  assert.ok(v.steps[2].card && v.steps[2].card.legs.length === 2, "Step 3 carries the placed 2-leg card");
...
test("DEMO: actionable next step — Lane A Step 3 placed (active card), Lane B queued restart carries a concrete candidate", () => {
  const a = buildPublicDualLadder(bb.laneA, "lane-a");
  const active = a.steps.find((s) => s.status === "active");
  assert.ok(active && active.card && active.card.legs.length >= 2, "Lane A Step 3 placed — active card with legs (not a blank row)");
  ...
  assert.ok(q.candidate && q.candidate.reason.length > 20, "queued restart carries a concrete candidate + reason");
  assert.ok(q.candidate.legs && q.candidate.legs.length === 2, "Lane B candidate shows its exact legs");
});

test("DEMO: only the PLACED card counts exposure ($100); candidates count nothing", () => {
  const mr = JSON.parse(fs.readFileSync("public/data/mr-dub/portfolio.json", "utf8"));
  assert.equal(mr.openExposure, 100, "Lane A Step 3 placed → the lane's $100 paper seed is at risk");
  // Lane B / Moonshot candidates are awaiting_approval — they add NO exposure until placed.
  const bbRaw = JSON.parse(fs.readFileSync("public/data/methodology/launch/dual-bank-builder-active.json", "utf8")).run;
  assert.equal(bbRaw.laneB.nextCandidate.status, "awaiting_approval", "Lane B candidate not placed (no exposure)");
});
