...
test("Bank Builder ACTIVE cross-slate + Lane B settled WON + Moonshot settled — real exposure; corrected bankroll + crown intact", () => {
  const p = read("public/data/mr-dub/portfolio.json");
  assert.equal(p.currentBankroll, 10176.17, "reconciled bankroll preserved (a won step rolls; pending cards don't realize)");
  assert.equal(p.openExposure, 100, "Lane A Step 3 seed placed; Lane B settled WON released its seed");
  assert.equal(p.totalOpenExposure, 100, "core $100; moonshot settled LOST → 0 open");
