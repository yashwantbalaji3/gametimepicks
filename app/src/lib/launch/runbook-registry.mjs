...
export const RUNBOOKS = Object.freeze({
  mlb: {
    ...
    products: { runs: "daily-products.yml (receipts + forward-coverage steps)", when: "by 8:00 AM ET, chained after the morning run (backstop 11:41 UTC)", quiet: "typed receipt per lane (NO_PLAY is healthy) + coverage artifact at the same stamp", recover: "FIRST npx tsx app/scripts/products/build-daily-product-receipts.mjs --now <iso> --dry-run (prints, writes nothing); then re-dispatch daily-products.yml — receipts are content-idempotent and duplicate runIds no-op", receipt: "data/internal/products/receipts/<date>.json (lifecycle+watchdog embedded) + data/internal/products/forward-coverage/<date>.json" },
