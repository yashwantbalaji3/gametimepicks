...
export const DISPOSITIONS = ["SHIPPED", "ALREADY_PROVEN", "REALITY_GATED", "FOUNDER_GATED", "ENGINEERING_OPEN"];
...
export const WORKSTREAMS = Object.freeze([
  {
    ...
    commit: null,
    note:
      "PARTIAL. The family exists (components/nav/date-sport-controls) and /simulate is migrated with SSR parity proven against the built export. NOT closed: Today, Picks, Parlay Center and the sport hubs have not adopted it, and /projections is a registered shrink-only exception — it keeps date AND sport in the query string, so closing it needs a `query` date mode on the owner plus re-verification of its four-step interactive experience.",
  },
  {
    id: 2,
...
export function executableNow(rows = WORKSTREAMS) {
  ...
  return rows.filter((r) => r.disposition === "ENGINEERING_OPEN" && r.dependsOn.every((d) => closed.has(d)));
}

/** The program's classification, derived rather than declared. */
export function programClassification(rows = WORKSTREAMS) {
  const open = rows.filter((r) => r.disposition === "ENGINEERING_OPEN");
