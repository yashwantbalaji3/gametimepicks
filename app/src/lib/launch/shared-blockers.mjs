...
export const SHARED_BLOCKERS = Object.freeze([
  ...
  },
  {
    ...
    engineeringEvidence: "founder approved Option A (first-party collector on gametime-picks) and 90-day retention on 2026-09-10; a PRIVATE Vercel Blob store (gtp-analytics) was created and linked to production + preview; api/collect.mjs writes through @vercel/blob with access private; api/analytics-retention.mjs (daily Vercel cron, CRON_SECRET-locked) deletes day buckets older than 90 days (api/_retention-core.mjs, unit-tested); the live collector validated synthetic payloads on production (allowed accepted; email key, free text, and a timestamp rejected; a foreign origin silently dropped); the privacy notice follows the build's analytics switch (lib/legal/texts.mjs); PRODUCTION PROOF 2026-09-11: two synthetic day-2026-01-01 events were stored in the private store and an on-demand run of the retention cron deleted both (store empty after); the browser switches (NEXT_PUBLIC_ANALYTICS_ENABLED + ENDPOINT) were set on production the same night — CLOSED on the first real events seen in the store · CLOSED 2026-09-10 on a real receipt: after the literal-env sink fix deployed (7b33aff38), a live production page view sent two first-party beacons (POST /api/collect/ → 204) and both events were stored in the private Blob store under analytics/2026-09-10/; the 90-day retention cron is scheduled (17 7 * * *) and was verified deleting expired folders",
