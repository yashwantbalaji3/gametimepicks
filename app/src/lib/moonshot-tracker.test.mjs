...
test("Moonshot lane artifact is stopped/LOST and its record is separate (0-1, not in core)", () => {
  ...
  assert.deepEqual(portfolio.record, { wins: 15, losses: 5, voids: 0, pending: 0 }, "core record after June-26 (Lane A Step-2 LOST; Lane B Step-1 WON) — moonshot not blended in");
});
...
test("protected crown is the cumulative banked total ($20,465.40 = two completed $100→$10k ladders), 15-5", () => {
  const portfolio = JSON.parse(read("public/data/mr-dub/portfolio.json"));
  ...
  assert.equal(portfolio.currentBankroll, 19965.4, "active bankroll = crown − $500 realized dual-lane losses (5 stopped seeds after June-26)");
});
