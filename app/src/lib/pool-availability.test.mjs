...
test("null payload → all sports absent", () => {
  ...
  assert.equal(p.nbaSingleGameOnly, false);
...
test("nbaSingleGameOnly: every NBA slip flagged singleGame → nbaSingleGameOnly=true", () => {
  // Live 2026-05-28 post-SGP case: NBA bucket has 12 slips, all
  // singleGame=true, generated by the explicit single-game path.
  const payload = {
    date: "2026-05-28",
    generatedAt: "2026-05-28T00:00:00Z",
    totalSlips: 12,
    sourcePools: { nbaCount: 91, mlbCount: 259 },
    buckets: {
      conservative: { nba: [], mlb: [], multi: [], all: [] },
      balanced: {
        nba: [{ singleGame: true }, { singleGame: true }, { singleGame: true }, { singleGame: true }],
        mlb: [],
        multi: [],
        all: [],
      },
  ...
  const p = classifyPoolAvailability(payload);
  assert.equal(p.nba, "present");
  assert.equal(p.nbaSingleGameOnly, true);
});

test("nbaSingleGameOnly: mix of single-game + multi-game slips → false", () => {
  const payload = {
    date: "2026-05-28",
    generatedAt: "2026-05-28T00:00:00Z",
    totalSlips: 4,
    sourcePools: { nbaCount: 91, mlbCount: 0 },
    buckets: {
      balanced: {
        nba: [{ singleGame: true }, { singleGame: false }],
        mlb: [],
        multi: [],
        all: [],
      },
...
test("nbaSingleGameOnly: NBA present but no singleGame flag → false", () => {
  const payload = {
    date: "2026-05-28",
    generatedAt: "2026-05-28T00:00:00Z",
    totalSlips: 4,
...
test("shouldRenderAvailabilityNote: fires for single-game-only NBA scenario", () => {
  const payload = {
    date: "2026-05-28",
    generatedAt: "2026-05-28T00:00:00Z",
    totalSlips: 4,
...
test("shouldRenderAvailabilityNote: silent on a fully-healthy slate", () => {
  const payload = {
    ...
    generatedAt: "2026-05-28T00:00:00Z",
    totalSlips: 9,
    sourcePools: { nbaCount: 100, mlbCount: 100 },
    buckets: {
      balanced: {
        nba: [{ singleGame: false }, { singleGame: false }],
        mlb: [{}, {}, {}],
        multi: [{}, {}],
        all: [{}, {}],
      },
  ...
  const p = classifyPoolAvailability(payload);
  assert.equal(p.nbaSingleGameOnly, false);
  assert.equal(shouldRenderAvailabilityNote(p), false);
});
