...
test("V2 · an absent market reads as a product sentence, and never as a bare `No price`", () => {
  ...
  assert.ok(absent.length > 0 || priced > 0,
  ...
  for (const { f, main } of renderedBoardPages()) {
    ...
    if (absent.some((s) => SHORT[s] && text.includes(SHORT[s]))) seenAbsent += 1;
    if (/<span class="gtp-pred-book[^"]*"[^>]*>(DraftKings|FanDuel|BetMGM|BetRivers|Fanatics|Caesars|ESPN BET)</.test(main)) seenBook += 1;
  ...
  if (absent.length) {
    assert.ok(seenAbsent > 0, `committed rows carry ${absent.join("/")} but no page rendered the wording — the state is produced and never reaches a reader`);
  }
  if (priced > 0) {
    assert.ok(seenBook > 0, `${priced} committed row(s) carry a captured price but no page names the sportsbook it came from — an unattributed price is not a fact`);
  }
});
