  ...
  return (
    <section aria-labelledby="top-model-picks" id="top-model-picks" className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {categories.map((c) => (
          <div key={c.market} className="rounded-[12px] px-3 py-3 flex flex-col gap-2" style={{ background: "color-mix(in srgb, var(--vault-scrim-base) 55%, transparent)", border: "1px solid var(--vault-border)" }}>
            <span className="flex items-center gap-2 font-mono uppercase tracking-[0.12em]" style={{ color: "var(--vault-text-mute)", fontSize: 9.5 }}>
              {c.label}
              {c.calibrationFailed ? (
                <span className="rounded px-1.5 py-0.5" style={{ background: "var(--vault-gold-dim)", color: "var(--vault-text-faint)", fontSize: 8 }}>settled record: model behind market</span>
              ) : null}
            </span>
