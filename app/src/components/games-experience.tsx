      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {filtered.map((g) => {
            ...
            return (
              <article key={g.id} className="gtp-card-hover rounded-[10px] px-4 py-4 flex flex-col gap-3" style={{ background: "rgba(11, 18, 14,0.55)", border: "1px solid var(--vault-border)", borderLeft: `3px solid ${id.accentVar}` }}>
                ...
                {g.signal ? (
                  <div className="rounded-[8px] px-2.5 py-2 flex items-center gap-2 min-w-0" style={{ background: "rgba(7, 11, 9,0.5)", border: "1px solid var(--vault-rule)" }}>
