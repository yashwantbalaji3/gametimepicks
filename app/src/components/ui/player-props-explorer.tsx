...
export default function PlayerPropsExplorer({ props }: { props: PublicProjection[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {anyPreLineup ? (
        <div className="flex items-center gap-2 rounded-[8px] px-3 py-2" style={{ background: "color-mix(in srgb, var(--vault-accent) 6%, transparent)", border: "1px solid color-mix(in srgb, var(--vault-accent) 25%, transparent)" }}>
      ...
      {market === "byplayer" ? (
        playerGroups.length > 0 ? (
          ...
          <div className="rounded-[10px] px-4 py-6 text-center" style={{ background: "color-mix(in srgb, var(--vault-scrim-base) 55%, transparent)", border: "1px solid var(--vault-border)" }}>
