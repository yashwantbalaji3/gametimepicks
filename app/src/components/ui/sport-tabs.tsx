          ...
          return (
            <button key={t.key} type="button" onClick={() => go(t.key)} aria-current={on ? "true" : undefined}
              ...
              {t.badge != null && t.badge !== "" ? (
                <span className="font-mono rounded-full px-1.5" style={{ background: on ? "var(--vault-gold-bright)" : "var(--vault-rule)", color: on ? "#0b0f1f" : "var(--vault-text-faint)", fontSize: 10 }}>{t.badge}</span>
              ) : null}
            </button>
          );
        })}
