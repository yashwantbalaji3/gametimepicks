...
function Chip({ label, value, tone }: { label: string; value: string; tone?: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1"
      style={{ background: "rgba(12,8,6,0.5)", border: "1px solid var(--vault-rule)" }}>
...
export default function GameScriptCard({ script, compact = false }: { script: GameScript; compact?: boolean }) {
  if (!script.available) {
    return (
      <div className="rounded-[10px] px-4 py-3" style={{ background: "rgba(26,16,11,0.5)", border: "1px solid var(--vault-border)" }}>
  ...
  return (
    <div className="rounded-[12px] px-4 py-3.5 flex flex-col gap-2.5"
      style={{ background: "rgba(26,16,11,0.6)", border: "1px solid var(--vault-gold-bright)", borderLeft: "3px solid var(--vault-gold-bright)" }}>
