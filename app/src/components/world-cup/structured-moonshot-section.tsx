...
const CONF: Record<string, string> = { High: "var(--vault-success)", Solid: "var(--vault-gold-bright)", Lean: "var(--vault-crown-warm)", Speculative: "var(--gtp-bank-heat)" };
const VOL: Record<string, string> = { Low: "var(--vault-success)", Medium: "var(--vault-gold-bright)", High: "var(--vault-crown-warm)", Extreme: "var(--gtp-bank-heat)" };

function Badge({ label, value, tone }: { label: string; value: string; tone: string }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5" style={{ border: `1px solid color-mix(in srgb, ${tone} 40%, transparent)`, background: "color-mix(in srgb, var(--vault-wash-base) 3%, transparent)" }}>
...
function GameGroup({ pair, index }: { pair: MoonshotGamePair; index: number }) {
  ...
  return (
    <div className="rounded-[9px] px-3 py-2.5 flex flex-col gap-2" style={{ background: "color-mix(in srgb, var(--vault-bg) 45%, transparent)", border: "1px solid var(--vault-rule)" }}>
      <div className="flex items-center justify-between gap-2 min-w-0">
        <span className="flex items-center gap-1.5 min-w-0">
          <span className="font-mono uppercase tracking-[0.12em]" style={{ color: "var(--vault-moonshot-soft)", fontSize: 8.5 }}>Game {letter}</span>
      ...
      <div className="flex flex-col gap-1">
        {pair.legs.map((l, i) => (
          <div key={i} className="flex items-center justify-between gap-2">
            <span className="flex items-center gap-1.5 min-w-0">
              <span className="font-mono uppercase tracking-[0.06em] rounded px-1 py-0.5" style={{ color: l.kind === "result" ? "var(--vault-gold-bright)" : "var(--vault-text-mute)", background: "color-mix(in srgb, var(--vault-wash-base) 3%, transparent)", fontSize: 7.5 }}>{l.kind === "result" ? "Result" : l.kind === "total" ? "Total" : "BTTS"}</span>
...
function TicketCard({ ticket }: { ticket: MoonshotTicket }) {
  if (!ticket.available) {
    return (
      <div className="rounded-[12px] px-4 py-3.5 flex flex-col gap-1.5" style={{ background: "color-mix(in srgb, var(--vault-scrim-base) 40%, transparent)", border: "1px dashed var(--vault-rule)", borderLeft: "2px solid var(--vault-moonshot)" }}>
  ...
  return (
    <div className="rounded-[12px] overflow-hidden flex flex-col" style={{ border: "1px solid var(--vault-rule)", background: "color-mix(in srgb, var(--vault-scrim-base) 50%, transparent)", borderLeft: "2px solid var(--vault-moonshot)" }}>
      <div className="px-4 py-3 flex flex-col gap-2" style={{ borderBottom: "1px solid var(--vault-rule)", background: "var(--vault-wash-faint)" }}>
      ...
      <div className="px-4 py-2.5 flex flex-col gap-1" style={{ borderTop: "1px solid var(--vault-rule)" }}>
        <span className="text-[10px] leading-snug" style={{ color: "var(--vault-text-mute)" }}><span className="font-mono uppercase tracking-[0.1em]" style={{ color: "var(--vault-moonshot-soft)", fontSize: 8.5 }}>Why it can hit · </span>{ticket.whyItCanHit}</span>
        <span className="text-[10px] leading-snug" style={{ color: "var(--vault-text-faint)" }}><span aria-hidden style={{ color: "var(--vault-crown-warm)" }}>⚠ Why it can miss · </span>{ticket.whyItCanFail}</span>
