...
const CONF: Record<string, string> = { High: "var(--vault-success)", Solid: "var(--vault-gold-bright)", Lean: "var(--vault-crown-warm)" };
...
function LaneCard({ lane }: { lane: ProposalLane }) {
  ...
  return (
    <div className="rounded-[12px] overflow-hidden flex flex-col" style={{ border: "1px solid var(--vault-rule)", background: "color-mix(in srgb, var(--vault-scrim-base) 50%, transparent)", borderLeft: "2px solid var(--vault-gold)" }}>
      <div className="px-4 py-3 flex flex-col gap-2" style={{ borderBottom: "1px solid var(--vault-rule)", background: "var(--vault-wash-faint)" }}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5" style={{ border: `1px solid color-mix(in srgb, ${CONF[lane.confidence]} 40%, transparent)`, background: "color-mix(in srgb, var(--vault-wash-base) 3%, transparent)" }}>
        ...
        {lane.legs.map((leg, i) => {
          ...
          <div key={i} className="flex items-center justify-between gap-2">
            <span className="flex items-center gap-1.5 min-w-0">
              ...
              <span className="font-mono uppercase tracking-[0.06em] rounded px-1 py-0.5" style={{ color: "var(--vault-text-faint)", background: "color-mix(in srgb, var(--vault-wash-base) 3%, transparent)", fontSize: 7.5 }}>{leg.marketLabel}</span>
            ...
            <span className="flex items-center gap-1.5 shrink-0">
              {st ? <span className="font-mono uppercase tracking-[0.06em] rounded px-1 py-0.5" style={{ color: st.tone, background: "color-mix(in srgb, var(--vault-wash-base) 3%, transparent)", border: `1px solid color-mix(in srgb, ${st.tone} 35%, transparent)`, fontSize: 7.5 }}>{st.label}</span> : null}
      ...
      <div className="px-4 py-2.5 flex flex-col gap-1" style={{ borderTop: "1px solid var(--vault-rule)" }}>
        ...
        <span className="text-[10px] leading-snug" style={{ color: "var(--vault-text-faint)" }}><span aria-hidden style={{ color: "var(--vault-crown-warm)" }}>⚠ Why it could fail · </span>{lane.whyItCouldFail}</span>
      </div>
    </div>
  );
}
...
export default function BankBuilderProposalCard({ proposal }: { proposal: BankBuilderProposal }) {
  ...
  return (
    <div className="rounded-[14px] px-4 py-4 flex flex-col gap-3" style={{ background: "color-mix(in srgb, var(--vault-scrim-base) 45%, transparent)", border: "1px solid var(--vault-gold)", borderLeft: "3px solid var(--vault-gold-bright)" }}>
      <div className="flex items-center justify-between gap-2">
