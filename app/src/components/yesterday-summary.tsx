    ...
    <section aria-label={`Results from ${dateLabel}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
        {tiles.map((t, i) => {
          ...
          const toneColor = t.tone === "win" ? "var(--gtp-success-on-dark)" : t.tone === "loss" ? "var(--vault-danger-soft)" : "var(--vault-text)";
          return (
            <Link
              key={t.key}
              href={t.href}
              className="gtp-fade-up gtp-card-hover flex items-center gap-2.5 rounded-[10px] px-3.5 py-3"
              style={{ background: "color-mix(in srgb, var(--vault-scrim-base) 55%, transparent)", border: "1px solid var(--vault-border)", textDecoration: "none", animationDelay: `${i * 60}ms` }}
            >
              <span
