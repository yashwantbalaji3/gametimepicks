...
export function SlateSummary({ dateLabel, mlbGames, mlbLeans, topPicks, bankBuilderStatus, moonshotStatus }: SlateSummaryProps) {
  return (
    <section aria-label="Today's slate summary" className="flex flex-col gap-3">
      ...
      <div className="rounded-[14px] px-4 py-3" style={{ background: "rgba(26,16,11,0.55)", border: "1px solid var(--vault-border)" }}>
...
function TrustTile({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col gap-0.5 rounded-[12px] px-3 py-2.5" style={{ background: "rgba(26,16,11,0.55)", border: "1px solid var(--vault-border)" }}>
  ...
  return (
    <section aria-label="How it works" className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
        {STEPS.map((s) => (
          <div key={s.n} className="flex gap-3 rounded-[12px] px-4 py-3.5" style={{ background: "rgba(26,16,11,0.4)", border: "1px solid var(--vault-rule)" }}>
...
export function FooterCta() {
  ...
  return (
    <section aria-label="Get started" className="flex flex-col items-center gap-3 rounded-[16px] px-5 py-7 text-center"
      style={{ border: "1px solid var(--vault-border-strong)", background: "linear-gradient(135deg, rgba(217,164,65,0.06), rgba(26,16,11,0.35))" }}>
