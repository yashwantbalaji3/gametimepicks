...
export default function ResultsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1280px] px-6 py-12">
      {/* Sample-data note (only renders when demo) — moved ABOVE the headline */}
      {hr.isDemo && (
        <aside className="surface px-4 py-3 mb-5 border-l-2 reveal" style={{ borderLeftColor: "var(--amber)" }}>
          <div className="flex items-start gap-3">
            ...
            <p className="font-mono text-[12px] text-[var(--text-mute)] leading-relaxed">
              {hr.sampleNote ||
                "These are sample historical-tracking numbers for demonstration. Real settlement begins once live games complete and the pipeline runs in non-demo mode. Not an actual track record."}
            </p>
          </div>
        </aside>
      )}
      ...
      <div className="reveal">
        <div className="eyebrow">{hr.isDemo ? "sample results — demo data" : "tracked results"}</div>
        <h1 className="mt-2 font-display text-[36px] md:text-[48px] tracking-tightest font-semibold leading-[1]">
          {formatPercent(o.hitRate)} <span className="text-[var(--text-mute)]">{hr.isDemo ? "sample hit rate" : "hit rate"}</span>
        </h1>
        <p className="mt-3 text-[var(--text-mute)] text-[14px] font-mono">
          {o.total} {hr.isDemo ? "sample leans" : "tracked leans"} · {hr.dateRange}
        </p>
      </div>

      {/* KPI tiles */}
      <section className="mt-8 grid grid-cols-2 md:grid-cols-5 gap-3">
        <KpiTile label={hr.isDemo ? "sample leans" : "leans"} value={String(o.total)} delay={1} />
        ...
        <KpiTile
          label={hr.isDemo ? "sample hit rate" : "hit rate"}
