#!/usr/bin/env node
/**
 * Compact NFL game history v2 for the live win + margin heads (P298).
 *
 * v1 (build-games-history.mjs) carries only each game's TOTAL, which is all the totals head needs. The
 * win and margin heads adopted from the P297 replay fold the winner, the margin and the venue, so v2 adds
 * both scores and the neutral-site flag. Same source file, same population, same franchise map, same
 * ordering fields — v1 is left byte-for-byte alone because the totals parity proof reads it.
 *
 * Usage: node scripts/research/nfl/build-games-history-v2.mjs
 * Reads:  data/internal/research/nfl/raw/nflverse/games.csv (hash must match the P297 registration)
 * Writes: data/internal/research/nfl/replay/games-history-v2.json
 */
...
const prereg = JSON.parse(fs.readFileSync(rel("data/internal/research/nfl/reports/win-margin-historical-replay-preregistration.json"), "utf8"));
const F = prereg.frozen;
const SRC = "data/internal/research/nfl/raw/nflverse/games.csv";
const OUT = "data/internal/research/nfl/replay/games-history-v2.json";
...
for (const line of lines.slice(1)) {
  ...
  games.push([r[col.game_id], r[col.espn] || null, season, r[col.gameday], franchise(r[col.home_team]), franchise(r[col.away_team]), hs, as, r[col.location] === "Neutral" ? 1 : 0, hs + as]);
}
...
fs.writeFileSync(rel(OUT), JSON.stringify({
  ...
  columns: ["gameId", "espnId", "season", "date", "home", "away", "homeScore", "awayScore", "neutral", "total"],
  games,
}));
