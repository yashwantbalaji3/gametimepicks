#!/usr/bin/env node
/**
 * NFL PLAYER PROPS — SHARE-LEVEL SECOND LOOK (P300)
 *
 * Executes data/internal/research/nfl/reports/player-props-share-level-preregistration.json. The engine is
 * P299's replay-player-props.mjs (left untouched, its sha is in the P299 receipt) with ONE model change: a
 * player's share carries no pull toward zero (frozen.share.shrinkK = 0). The share-times-volume baseline
 * keeps v1's shrinkK (frozen.share.baselineShrinkK). Adds the level bar P299 lacked. MAE scores the predictive
 * MEDIAN (the point that minimises absolute error); the level bar scores the predictive MEAN.
 *
 *   --validate            DEV seasons only (2022–2025). A metric call on a non-dev row throws.
 *   --score --now <ISO>   the ONE second look at 2014–2021. Refuses unless the registration is committed and
 *                         unmodified, and refuses if the output already exists. Those checks run FIRST.
 *
...
const OUT_PATH = "data/internal/research/nfl/reports/player-props-share-level-second-look.json";
...
if (MODE === "score") {
  ...
  if (fs.existsSync(rel(OUT_PATH))) refuse(`${OUT_PATH} already exists — the second look has been taken`);
...
/** P300: the model share uses frozen.share.shrinkK (0 = no pull toward zero); the baseline passes v1's value. */
    ...
    for (const tg of teamGames.values()) {
      ...
      for (const id of byTeam.get(tg.team) ?? []) {
        ...
        for (const mkt of MARKETS) {
          ...
          const base = { season, mkt, actual: actualOf[mkt](row), rolling4, shareVol: baselineShare * INTERCEPT[volKey] * perOpp[mkt], isCount: mkt === "player_receptions" };
...
function metrics(list) {
  ...
  return {
    n,
    mae: avg((x) => Math.abs(x.p50 - x.actual)), // P300: MAE scores the predictive median (the MAE-optimal point)
    ...
    meanForecast,
    meanActual,
    level: meanActual > 0 ? meanForecast / meanActual : null,
  };
}
...
const line = (label, m) => `${label} n ${m.n} · mae ${m.mae.toFixed(3)} · rolling4 ${m.rolling4Mae.toFixed(3)} · shareVol ${m.shareVolMae.toFixed(3)} · cov80 ${m.coverage80.toFixed(3)} · ece ${m.ece?.toFixed(3)} · level ${m.level?.toFixed(3)}`;

if (MODE === "validate") {
  console.log(`scored rows (all seasons): ${scored.length} · pfr-prefixed histories ${counts.pfrHistories}`);
  ...
  for (const mkt of MARKETS) for (const cand of CANDS) {
    for (const season of [2022, 2023, 2024, 2025]) {
      ...
      if (m) console.log(line(`${season} ${mkt.padEnd(21)} ${cand.padEnd(15)}`, m));
    }
    const all = metrics(slice(cand, mkt, isDev));
    if (all) console.log(`${line(`DEV  ${mkt.padEnd(21)} ${cand.padEnd(15)} s ${scale[`${mkt}|${cand}`]} ·`, all)}\n`);
  }
  process.exit(0);
}

// ── --score: the one second look ──────────────────────────────────────────────────────────────────
...
function bootstrap(list) {
  ...
  for (const x of list) { const o = agg.get(x.season); o.sum += Math.abs(x.p50 - x.actual) - Math.abs(x[better] - x.actual); o.n += 1; }
...
const perEra = (eras, f) => Object.fromEntries(Object.entries(eras).map(([k, m]) => [k, m ? f(m) : null]));
for (const mkt of MARKETS) {
  results[mkt] = {};
  bars[mkt] = {};
  verdicts[mkt] = {};
  for (const cand of CANDS) {
    ...
    results[mkt][cand] = { evidenceTier: "SECOND_LOOK", dispersionScale: scale[`${mkt}|${cand}`], overall, eras, seasons, bootstrap: boot };
    const [lo, hi] = F.bars.coverageBand;
    const [lvLo, lvHi] = F.bars.levelBand;
    const inLevel = (m) => m.level != null && m.level >= lvLo && m.level <= lvHi;
    const allEras = (f) => Object.values(eras).every((m) => m && f(m));
    const b = overall ? {
      beatsRolling4: bar(overall.mae < overall.rolling4Mae && allEras((m) => m.mae < m.rolling4Mae), "below rolling-4 overall and in every era", { overall: [overall.mae, overall.rolling4Mae], eras: perEra(eras, (m) => [m.mae, m.rolling4Mae]) }),
      beatsShareVolume: bar(overall.mae < overall.shareVolMae && allEras((m) => m.mae < m.shareVolMae), "below share x volume overall and in every era", { overall: [overall.mae, overall.shareVolMae], eras: perEra(eras, (m) => [m.mae, m.shareVolMae]) }),
      ...
      thresholdCalibration: bar(overall.ece != null && overall.ece <= F.bars.eceMax && allEras((m) => m.ece != null && m.ece <= F.bars.eceEraMax), `ECE <= ${F.bars.eceMax} overall and <= ${F.bars.eceEraMax} in every era`, { overall: overall.ece, eras: perEra(eras, (m) => m.ece) }),
      level: bar(inLevel(overall) && allEras(inLevel), `mean forecast / mean actual in [${lvLo}, ${lvHi}] overall and in every era`, { overall: overall.level, eras: perEra(eras, (m) => m.level) }),
      minimumN: bar(overall.n >= F.bars.minimumN, `n >= ${F.bars.minimumN}`, overall.n),
    } : { minimumN: bar(false, `n >= ${F.bars.minimumN}`, 0) };
    bars[mkt][cand] = b;
    verdicts[mkt][cand] = Object.values(b).every((x) => x.pass) ? "SECOND_LOOK_ELIGIBLE" : "SECOND_LOOK_REJECTED";
  }
}

const receipt = {
  schemaVersion: 1,
  artifact: "player-props-share-level-second-look",
  dataClass: "PRIVATE_RESEARCH",
  program: "300",
  evidenceTier: "SECOND_LOOK — 2014-2021 was scored once by P299 and its aggregate figures were known to the author; nothing here is a blind held-out result",
  generatedAt: NOW,
  preregistration: { path: PREREG_PATH, commit: preregCommit },
  scriptSha256: sha("scripts/research/nfl/replay-player-props-v2.mjs"),
  population: {
  ...
  bars,
  verdicts,
  consequence: "Nothing publishes from this receipt. The live board keeps its share rule; changing it for an eligible family is a founder-approved app/ step, proposed with this evidence tier stated, and re-read against the blind 2026 forward test.",
};
...
for (const mkt of MARKETS) for (const cand of CANDS) {
  ...
  console.log(`${line(`${mkt.padEnd(21)} ${verdicts[mkt][cand].padEnd(20)} s ${scale[`${mkt}|${cand}`]} ·`, o)} · failed: ${Object.entries(bars[mkt][cand]).filter(([, b]) => !b.pass).map(([k]) => k).join(", ") || "none"}`);
}
