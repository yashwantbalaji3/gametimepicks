...
|----------|---------------|
| [`METHODOLOGY_V2_POST_SETTLE_CHECKLIST.md`](./METHODOLOGY_V2_POST_SETTLE_CHECKLIST.md) (operator checklist) | Docs-only 5-step checklist to run after each `nightly-settle`: verify the new graded slate exists → run the shadow report (`--write-report`) → inspect L5 5/5 / Low sample sizes → PR the report if it changed → keep live v2 blocked until both gated buckets reach ≥40 decided legs. No product change. |
| [`METHODOLOGY_V2_SHADOW_TRACKING_RUNBOOK.md`](./METHODOLOGY_V2_SHADOW_TRACKING_RUNBOOK.md) (runbook + tooling) | How to keep tracking v2 evidence as slates settle — no live change. `scripts/shadow-parlay-methodology-v2.mjs` now **auto-discovers** settled slates (public era ≥ 2026-05-27, banned May 25/26 excluded, June-2+ only once graded) and takes **`--write-report`** → deterministic `audits/methodology-v2-shadow-latest.md`. Threshold: ≥40 decided legs in L5 5/5 / Low (now 17 / 14 → keep shadow-only). |
| [`METHODOLOGY_V2_IMPLEMENTATION_DECISION_2026-06-02.md`](./METHODOLOGY_V2_IMPLEMENTATION_DECISION_2026-06-02.md) (decision memo) | Post-`#258` decision matrix (7 options A–G) + approval gates for any live v2. On corrected June-2 data the truncation gap is closed, but the high-signal rules stay thin (L5 5/5 N=17, Low N=14), L5 4/5+ shows no edge (52%=baseline), and ~15 cards conflict with the `#241` caps. **Recommendation: keep v2 shadow-only + gather ~7–14 more settled slates (Option A/G); no narrow live increment clears the bar — Bank Builder already matches the desired direction and a hard L10 gate would starve. Nothing wired.** |
| [`SUGGESTED_PARLAY_METHODOLOGY_V2_2026-06-02.md`](./SUGGESTED_PARLAY_METHODOLOGY_V2_2026-06-02.md) (spec + **shadow v2**) | Shadow-audits proposed Methodology v2 (L5/L10 leg quality + per-section odds caps + ~15-card targets) via `scripts/shadow-parlay-methodology-v2.mjs`. **Headline data bug:** published `recentSeries` is `series[:10]` = the **OLDEST 10** games for ≈88% of MLB legs (optimizer truncation; the model itself uses the recent tail). True L5/L10 must come from the **board full series** — on June-2, **28 legs flip Low-eligibility** (true Low 21 vs truncated 11). **Signal:** L5 5/5 = 71% & Low(5/5 & ≤−150) = 79% but **N=17/14 (inconclusive)**; **L5 4/5+ = 52% = baseline (no edge)**; Bank L10≥8/10 = 63% (N=64). Targets reachable from the real pool (relaxed→16/16) but the live `#241` caps (totalMax 9) hold it to 8. **Verdict: SHADOW-ONLY — fix the truncation at source + grow the sample before any live wiring. Nothing wired.** **Follow-up: the source truncation is fixed forward in `fix/optimizer-recentseries-recency` (`last_n_recent_values` → `series[-10:]`), and the June-2 optimizer was regenerated in `regen/june2-recentseries` (recentSeries-only diff; truncated-vs-true gap closed to 0 flips, was 28). v2 stays shadow-only — still blocked on thin settled samples + the #241 cap-vs-target decision.** |
