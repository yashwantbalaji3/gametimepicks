...
- **Fully wired (validated against the first 565 real observations):** `team_offensive_form` coverage key added to `simulation-feature-contract.ts` (13→14 families); `build-mlb-research-observations.mjs` now attaches the home/away record per-team (additive, only when `researchEligible`) with a `hasTeamOffensiveForm` flag. On 07-22 it reads **absent (0%)** — the capture script was added mid-slate, so no eligible pregame record existed — which is honest; **future** dates will carry it. Quality gate stayed PASS; tsc clean.
